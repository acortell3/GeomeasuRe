...
is_oriented <- function(x){
  ...
  l <- sp::SpatialLines(list(sp::Lines(list(sp::Line(cbind(c(cent,cent),c(0,10)))),ID = "line")),proj4string = Prj)
  inters <- rgeos::gIntersection(x,l) ## intersect polygon
  li <- rgeos::gBuffer(inters, width = 0.000001) ## Create tiny polygon to divide
  two_p <- rgeos::gDifference(x,li) ## Differentiate polygons
...
L_rel <- function(x,y){
  ...
  ## Recreate the spatial object
  sp <- sp::SpatialPolygons(list(sp::Polygons(list(sp::Polygon(cbind(coordx,coordy))),1)),proj4string = Prj)
  x <- sp::SpatialPolygonsDataFrame(sp,x@data,match.ID = FALSE)
  ...
  ## Recreate the spatial object
  sp <- sp::SpatialPolygons(list(sp::Polygons(list(sp::Polygon(cbind(coordx,coordy))),1)),proj4string = Prj)
  y <- sp::SpatialPolygonsDataFrame(sp,x@data,match.ID = FALSE)
